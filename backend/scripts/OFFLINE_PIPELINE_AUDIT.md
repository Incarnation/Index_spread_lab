...
- [x] `python -m backend.scripts.backtest.cli --csv ...` produces byte-identical optimizer output (same caveat: covered by the bytecode + CSV regression).  `--help` on the `backtest` CLI now also renders cleanly: a pre-existing latent bug in the verbatim header (a literal `%+` in the `--optimize-selective` help string was misinterpreted as a printf format specifier by argparse) was fixed in the Wave-5 close-out commit (`90%+` → `90%%+` in [`backend/scripts/backtest/cli.py:291`](backtest/cli.py)). The bug existed in the original monolith too but was never reachable because nobody invoked `--help` until the package-CLI smoke-tests added the `__main__` guard. The bytecode regression baseline ([`tools/_regression/backtest_after.json`](../../tools/_regression/backtest_after.json)) was re-cut to absorb the one-character help-string change; the pre-split `before.json` → post-fix `after.json` diff now shows 1 deliberate change in `main` (the changed help-string constant) -- xgb and training baselines remain perfectly byte-identical. Real invocations (`--optimize`, `--walkforward`, etc.) were unaffected.
...
- [x] Backtest `EventSignalDetector` is a thin wrapper around `services/event_signals.py`. Verified in the Wave-5 close-out: [`backend/scripts/backtest/engine.py:626-686`](backtest/engine.py) defines the class with the docstring *"Thin wrapper around the shared `evaluate_event_signals` evaluator … the actual signal-firing rules live in one place so the backtest can never silently drift from live."* `detect()` calls `evaluate_event_signals(ctx, self._build_thresholds(), log_warnings=False)` directly — no duplicated rule logic remains. The previous "still partial" note was stale.
- [x] [`OFFLINE_ML.md`](../spx_backend/jobs/OFFLINE_ML.md) reviewed by operator. The doc was authored during Wave 5 (240 lines, complete contract spec) and reconciled in the close-out commit: §3.1 producer paths now point at the post-split [`backend/scripts/xgb/`](xgb/) package; §4.1 missing-feature coercion documented as `NaN` (matching [`modeling.py:1147`](../spx_backend/jobs/modeling.py)) instead of the previously-claimed `0.0`; §7 grew a new bullet flagging the deferred-Track-B `chosen_legs_json` shape mismatch finding below.

#### Deferred Track B follow-ups (surfaced during Wave-5 close-out)

- [ ] **`chosen_legs_json` shape mismatch.** Live [`decision_job._build_candidate`](../spx_backend/jobs/decision_job.py) (lines 678-715) writes the chosen legs at the **top level** of `chosen_legs_json` (`{"short": {...}, "long": {...}, ...}`), but [`extract_xgb_features`](../spx_backend/jobs/modeling.py) reads them from a **nested** `legs.short` / `legs.long` path with a flat `short_iv` / `long_iv` fallback (lines 995-1006). Neither path matches the live shape, so leg-level features (`short_iv`, `long_iv`, `short_delta`, `long_delta`, `iv_skew_ratio`, `credit_to_max_loss`) silently fall through to `None` → `NaN` if XGB scoring were re-enabled today. **Recommended fix:** extend `extract_xgb_features` to also try the top-level shape before falling back to nested or flat. Documented in [`OFFLINE_ML.md §7`](../spx_backend/jobs/OFFLINE_ML.md). Track this **before** flipping any V1/V2 model to `is_active=TRUE`.
